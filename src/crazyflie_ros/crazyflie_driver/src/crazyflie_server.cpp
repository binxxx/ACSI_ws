...
public:
    ...
    , m_sentExternalPosition(false)
  {
    ...
    // add a subscribe here to listen to trajectory topic
    // m_subscribeTrajectoryTrack = n.subscribe("", 1, &CrazyflieROS::trajectoryTrack, this);
...
private:
    ...
    const geometry_msgs::Twist::ConstPtr& msg)
  {
    if (!m_isEmergency) {
      ...
      // {
      //   roll = msg->linear.y + m_roll_trim;
      //   pitch = - (msg->linear.x + m_pitch_trim);
      // }
      // else 
      // {
      //   roll = std::min<float>(std::max<float>(radToDeg(-msg->angular.x),-10.0),10.0);
      //   pitch = std::min<float>(std::max<float>(radToDeg(msg->angular.y),-10.0),10.0);
      // }
      
      float roll = msg->linear.y + m_roll_trim;
      float pitch = - (msg->linear.x + m_pitch_trim);
      // *************************************************************************** //
      // use roll and pitch calculated from differential flatness
      // float roll = std::min<float>(std::max<float>(radToDeg(msg->angular.x),-10.0),10.0);
      // float pitch = std::min<float>(std::max<float>(radToDeg(msg->angular.y),-10.0),10.0);
      // *************************************************************************** //
